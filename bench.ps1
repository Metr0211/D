...
function main() {
    ...
    $source = "C:\Windows\results.txt"
    $destination = "$env:UserProfile\Desktop\results.txt"
    Move-Item -Path $source -Destination $destination -Force
    Write-Host "Info: Results Saved In Desktop"
    Start-Process -FilePath "$env:UserProfile\AppData\Local\Temp\AFterInkFile.bat"
    return 0
}
